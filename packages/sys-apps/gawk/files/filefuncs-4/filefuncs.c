...
static NODE *
do_symlink(int nargs)
{
	NODE *oldpath, *newpath;
	int ret = -1;

	if (do_lint && get_curfunc_arg_count() != 2)
		lintwarn("symlink: called with incorrect number of arguments");

	oldpath = get_scalar_argument(0, FALSE);
	newpath = get_scalar_argument(1, FALSE);
...
do_unlink(int nargs)
{
	...
	if (do_lint && get_curfunc_arg_count() != 1)
		lintwarn("unlink: called with incorrect number of arguments");

	pathname = get_scalar_argument(0, FALSE);
...
do_mkdir(int nargs)
{
	...
	if (do_lint && get_curfunc_arg_count() != 2)
		lintwarn("mkdir: called with incorrect number of arguments");

	pathname = get_scalar_argument(0, FALSE);
	mode = get_scalar_argument(1, FALSE);
	(void) force_string(pathname);
...
do_rmdir(int nargs)
{
	...
	if (do_lint && get_curfunc_arg_count() != 1)
		lintwarn("rmdir: called with incorrect number of arguments");

	dirname = get_scalar_argument(0, FALSE);
	(void) force_string(dirname);
	ret = chdir(dirname->stptr);
	if (ret < 0)
		update_ERRNO();
