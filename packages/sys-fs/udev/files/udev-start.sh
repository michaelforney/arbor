...
write_root_symlink_rule()
{
   ...
   [[ ${ROOT_MAJOR} -gt 0 ]] || return
    ...
    mkdir -m 0755 -p /run/udev/rules.d

   # XXX for some reason, using a heredoc fails, so lets go with echo
   echo "# autogenerated by ${0}" > /run/udev/rules.d/10-root-symlink.rules
   echo 'ACTION=="add|change", SUBSYSTEM=="block", ENV{MAJOR}=="${ROOT_MAJOR}", ENV{MINOR}=="${ROOT_MINOR}", SYMLINK+="root"' >> /run/udev/rules.d/10-root-symlink.rules
}
